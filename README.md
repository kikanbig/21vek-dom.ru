# 21vek-dom.ru - Мульти-репозиторий конструктор сайта 🏗️

[![GitHub](https://img.shields.io/badge/GitHub-21vek--dom.ru-blue)](https://github.com/kikanbig/21vek-dom.ru)
[![React](https://img.shields.io/badge/React-18.3.1-blue)](https://reactjs.org/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.8.3-blue)](https://www.typescriptlang.org/)
[![Vite](https://img.shields.io/badge/Vite-5.4.19-yellow)](https://vitejs.dev/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-3.4.17-38B2AC)](https://tailwindcss.com/)

## 🎯 О проекте

**21vek-dom.ru** - это инновационный мульти-репозиторий конструктор сайта, где каждый GitHub репозиторий становится отдельной страницей или разделом. Главная страница построена на базе [hoff-divan-insights](https://github.com/kikanbig/hoff-divan-insights) - полнофункционального мебельного интернет-магазина.

## 🏛️ Архитектура

### Основной принцип
- **Главная страница (`/`)**: hoff-divan-insights (мебельный интернет-магазин)
- **Будущие страницы**: каждый новый репозиторий = новая страница
- **Централизованный репозиторий**: все интегрированные репозитории в одном месте

### Текущая структура
```
21vek-dom.ru/
├── src/
│   ├── components/     # Компоненты из интегрированных репозиториев
│   ├── pages/         # Страницы (Index, NotFound, будущие)
│   ├── hooks/         # Переиспользуемые хуки
│   ├── lib/           # Утилиты
│   └── assets/        # Ресурсы из всех репозиториев
├── repos/            # Клонированные репозитории (для разработки)
└── PROJECT_RULES.md  # Правила интеграции новых репозиториев
```

## 🚀 Быстрый старт

### Установка зависимостей
```bash
npm install
```

### Запуск в режиме разработки
```bash
npm run dev
```
Сервер запустится на `http://localhost:8080`

### Сборка для продакшена
```bash
npm run build
npm run preview
```

## 🔄 Синхронизация репозиториев

Система позволяет легко синхронизировать изменения из оригинальных репозиториев:

### Ручная синхронизация
```bash
# Синхронизация всех репозиториев
npm run sync:all

# Синхронизация конкретных репозиториев
npm run sync:hoff        # Главная страница (hoff-divan-insights)
npm run sync:sets        # Страница сетов

# Синхронизация с предварительным просмотром изменений
npm run sync:hoff-diff   # hoff-divan-insights с диффом
npm run sync:sets-diff   # sets-repo с диффом

# Синхронизация отдельных компонентов
npm run sync:sets-card   # Только SetCard компонент
```

### Ручная синхронизация
- **По требованию**: Синхронизация при необходимости через npm скрипты
- **Проверка статуса**: `npm run sync:status` для проверки актуальности репозиториев
- **Полная синхронизация**: `npm run sync:all` для обновления всех репозиториев

## 🛡️ Система сохранения кастомных модификаций

Система автоматически сохраняет проект-специфичные изменения после каждой синхронизации:

### Защищенные модификации
- ✅ **Навигация**: Ссылки на интегрированные страницы (`/sets`)
- ✅ **Компоненты**: Кастомная логика и UI элементы
- ✅ **Конфигурации**: Проектные настройки и переменные

### Автоматическое восстановление
После каждой синхронизации система:
1. Применяет сохраненные модификации
2. Проверяет целостность кода
3. Тестирует сборку проекта

```bash
# Ручное применение модификаций
npm run apply-mods
```

## 🛠️ Технологии

- **Frontend**: React 18 + TypeScript
- **Сборка**: Vite + SWC
- **Стилизация**: Tailwind CSS + shadcn/ui
- **Маршрутизация**: React Router v6
- **Управление состоянием**: React Query (TanStack Query)
- **UI компоненты**: Radix UI + Lucide Icons
- **Развертывание**: Railway CLI

## 📋 Правила проекта

### ⚠️ Добавление нового репозитория

**ВАЖНО:** При добавлении нового репозитория используйте подробный чек-лист:
- 📋 **[INTEGRATION_CHECKLIST.md](./INTEGRATION_CHECKLIST.md)** - пошаговая инструкция с примерами

**Критически важно** (без этого синхронизация НЕ РАБОТАЕТ):
1. ✅ Добавить в `config/repos.json`
2. ✅ Добавить скрипты в `package.json` (`sync:<имя>`, обновить `sync:all`)
3. ✅ Добавить Route в `src/App.tsx`

### Краткий чек-лист для добавления нового репозитория:

1. **Клонировать** репозиторий
2. **Скопировать** файлы в проект
3. **Добавить Route** в `src/App.tsx`
4. **⚠️ Добавить в `config/repos.json`** (иначе не будет синхронизироваться!)
5. **⚠️ Добавить скрипты в `package.json`** (иначе не будет синхронизироваться!)
6. **Обновить** документацию
7. **Собрать и задеплоить**

Подробные правила интеграции описаны в [`PROJECT_RULES.md`](./PROJECT_RULES.md).

## 📦 Интегрированные репозитории

| Репозиторий | Статус | Маршрут | Описание |
|-------------|--------|---------|----------|
| [hoff-divan-insights](https://github.com/kikanbig/hoff-divan-insights) | ✅ Интегрирован | `/` | Главная страница - мебельный интернет-магазин |
| [project-2870847f-5dea-434d-9cca-4fbca27ed98e](https://github.com/kikanbig/project-2870847f-5dea-434d-9cca-4fbca27ed98e.git) | ✅ Интегрирован | `/sets` | Страница сетов для держателей карт «Любимый покупатель» |
| [opening](https://github.com/kikanbig/opening.git) | ✅ Интегрирован | `/opening` | Страница открытия магазина с видео и фотогалереей |
| *Следующие репозитории...* | 🔄 Ожидают | `/*` | Будущие страницы сайта |

## 📚 Подробная документация

Для получения полной информации о:
- **Пошаговой интеграции** новых репозиториев
- **Системе синхронизации** изменений
- **Правилах разработки** и архитектуры
- **SEO оптимизации** и производительности
- **Безопасности** и мониторинга

Ознакомьтесь с **[PROJECT_RULES.md](./PROJECT_RULES.md)** - подробным руководством по проекту.

## 🎨 Особенности главной страницы

Главная страница включает:
- **Hero секцию** с баннером и призывом к действию
- **Категории товаров** (диваны, кровати, шкафы, кухни, офис)
- **Секцию товаров** с карточками продукции
- **Адаптивный дизайн** для всех устройств
- **Современный UI** с shadcn/ui компонентами

## 🔧 Разработка

### Добавление новых компонентов
```bash
# Используйте shadcn/ui CLI для новых компонентов
npx shadcn-ui@latest add button
```

### Структура компонентов
- `src/components/ui/` - shadcn/ui компоненты
- `src/components/` - бизнес-компоненты
- `src/pages/` - страницы приложения

### Стилизация
- Используйте **Tailwind CSS** классы
- Кастомные CSS переменные в `src/index.css`
- Темная тема поддерживается

## 🚀 Деплой

Проект настроен для **ручного развертывания** на **Railway**:

```bash
# Полный деплой (сборка + загрузка)
npm run deploy

# Или по шагам:
npm run build    # Сборка проекта
railway up       # Загрузка на Railway
```

### Настройка Railway
```bash
# Связывание с проектом
railway link --project 27e6666f-49d1-4fbb-993c-a861a61af645

# Проверка статуса
railway status

# Просмотр логов
railway logs
```

### Доступ к проекту
- **Railway домен**: `https://kitchen-sets-landing-production.up.railway.app`
- **Основной домен**: `https://21vek-dom.ru` (при настройке кастомного домена)

## 📝 Лицензия

Этот проект создан для демонстрации концепции мульти-репозиторий конструктора.

## 🤝 Вклад в развитие

1. Форкните репозиторий
2. Создайте feature branch (`git checkout -b feature/amazing-feature`)
3. Зафиксируйте изменения (`git commit -m 'Add amazing feature'`)
4. Запушьте branch (`git push origin feature/amazing-feature`)
5. Создайте Pull Request

## 📞 Контакты

**Автор**: [@kikanbig](https://github.com/kikanbig)

---

*Этот проект демонстрирует современный подход к разработке веб-приложений с использованием модульной архитектуры и интеграции нескольких репозиториев.*
